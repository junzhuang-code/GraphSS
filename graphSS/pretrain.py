    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if acc_val > best_acc:
            ...
            save_checkpoint(
                    ...
                    is_best = is_best, \
                    directory = dirs, \
                    #filename = "epoch%d_acc%f.pth.tar"%(epoch, best_acc)
                    filename = FileName
                            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        model_name = "SGC" # GCN, SGC, GraphSAGE, TAGCN, GIN
        NUM_EPOCHS = 200
        GPU = -1
        NOISY_RATIO = 0.5
        is_trainable = True
    ...
    if is_trainable:
        ...
        dump_pickle('../data/attacker_data/W_{0}_{1}.pkl'\
                    .format(data_name, model_name), [W0.detach().numpy(), W1.detach().numpy()])
    ...
    #print("Distribution of Groundtruth: \n", Counter(list(tensor2array(label, GPU))))
    #print("Distribution of Prediction: \n", Counter(list(tensor2array(Y_pred, GPU))))
